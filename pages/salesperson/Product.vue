<template>
	<view class="user_list">
		...
		<view v-show="isLoadMore" v-if="productList.length != 0">
...
<script>	
	export default {
		...
		methods: {
			...
			jumpProductModify(product) {
				this.$api.http.get('/product/getImage?id='+product.id, null).then(res => {
					...
					if (this.noClick) {
						this.noClick = false;
						uni.navigateTo({
							url: '../product/ProductModify?product='+encodeURIComponent(JSON.stringify(product))
						})
					}
				})
			},
			...
			getProductList() {
				this.$api.http.get('/product/findAll', this.productRequest).then(res => {
					this.productList = this.productList.concat(res.content);
					if (res.numberOfElements < this.productRequest.size) {
						this.isLoadMore = true
						this.loadStatus = 'nomore'
					} else {
						this.isLoadMore = false
					...
					this.isLoadMore = true
					if (this.productRequest.page > 1) this.productRequest.page -= 1
				})	
			},
			search() {
				this.isSearch = true;
